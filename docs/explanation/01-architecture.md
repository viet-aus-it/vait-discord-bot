...
Each entrypoint — `processInteraction` for Discord commands, `processMessage` for message handlers, or a bin script's `main` function — creates a single root span. The event handlers in `bin/main.ts` wrap their callbacks in `ROOT_CONTEXT` so each unit of work starts from a pristine root context, unaffected by any span context inherited from the bot's startup async scope (e.g. the Discord gateway connection). Individual command handlers then enrich that span with domain-specific attributes (e.g., `bot.rep.new_value`, `bot.weather.location`) via `setSpanAttributes()`, and record errors on the span via `recordSpanError()`. Fatal crashes (uncaught exceptions, unhandled rejections) get their own `fatal-*` spans before the process exits. No child spans are created manually.
