...
export const execute: SlashCommandHandler = async (interaction) => {
  return tracer.startActiveSpan('command.serverSettings.reminderChannel', async (span) => {
    try {
      ...
      if (op.isErr()) {
        logger.error(`[set-reminder-channel]: ${interaction.member!.user.username} failed to set reminder channel to ${channel.name}`, {
          error: op.unwrapErr(),
        });
