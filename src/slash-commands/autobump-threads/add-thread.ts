...
export const addAutobumpThreadCommand: SlashCommandHandler = async (interaction) => {
  return tracer.startActiveSpan('command.autobumpThreads.add', async (span) => {
    try {
      ...
      if (op.isErr()) {
        logger.error(`[add-autobump-thread]: Cannot save thread ${thread.id} to be autobumped for guild ${guildId}`, { error: op.unwrapErr() });
