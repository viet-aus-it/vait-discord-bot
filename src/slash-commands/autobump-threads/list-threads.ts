...
export const listAutobumpThreadsCommand: SlashCommandHandler = async (interaction) => {
  return tracer.startActiveSpan('command.autobumpThreads.list', async (span) => {
    try {
      ...
      if (threads.isErr()) {
        logger.error(`[list-autobump-threads]: Cannot get list of threads from the database for guild ${guildId}`, { error: threads.unwrapErr() });
