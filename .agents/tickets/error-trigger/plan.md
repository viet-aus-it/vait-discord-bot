...
After: `logger.error` without span = `scripts/*` (manual ops, §3.5) + `add-thread` not-a-thread (legitimate error, kept `error`; carries `bot.autobump.reason`). Log-severity trigger clean of expected-state noise.
...
Log-only legitimate errors (no error span, e.g. `add-thread` not-a-thread) set `bot.<op>.reason` string alongside `success: false` so the failure stays attributable.
